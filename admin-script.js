...
function checkAdminAccess() {
    const adminSession = sessionStorage.getItem('aviationStoreAdmin');
...
function logout() {
    if (confirm('Êtes-vous sûr de vouloir vous déconnecter ?')) {
        sessionStorage.removeItem('aviationStoreAdmin');
        window.location.href = 'login.html';
    }
}
...
function getSalesData() {
    return JSON.parse(localStorage.getItem('aviationStoreSales') || '[]');
}
...
function generateSampleData() {
    ...
    localStorage.setItem('aviationStoreSales', JSON.stringify(sampleData));
...
function clearAllData() {
    if (confirm('Êtes-vous sûr de vouloir supprimer toutes les données de vente? Cette action est irréversible.')) {
        localStorage.removeItem('aviationStoreSales');
        refreshData();
...
function getProducts() {
    return JSON.parse(localStorage.getItem('aviationStoreProducts')) || {
...
function saveProducts(products) {
    localStorage.setItem('aviationStoreProducts', JSON.stringify(products));
...
function saveProduct(event) {
    ...
    if (productId) {
        ...
        showMessage(`Produit "${productName}" modifié avec succès!`, 'success');
    } else {
        // Add new product
        const newId = Math.max(...Object.keys(products).map(Number)) + 1;
        products[newId] = productData;
        const productName = String(productData.name);
        showMessage(`Produit "${productName}" ajouté avec succès!`, 'success');
    }
